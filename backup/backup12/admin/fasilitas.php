        ...
        <div :class="{'dark text-white-dark' : $store.app.semidark}">
            <nav x-data="sidebar" class="sidebar fixed top-0 bottom-0 z-50 h-full min-h-screen w-[260px] shadow-[5px_0_25px_0_rgba(94,92,154,0.1)] transition-all duration-300">
                <div class="h-full bg-white dark:bg-[#0e1726]">
                    ...
                    <ul class="perfect-scrollbar relative h-[calc(100vh-80px)] space-y-0.5 overflow-y-auto overflow-x-hidden p-4 py-0 font-semibold" x-data="{ activeDropdown: 'datatables' }">
                                ...
                                <div class="p-5">
                                    <form>
                                        ...
                                        <select class="form-select mb-3">
                                            ...
                                            <?php
                                            $ambildata = mysqli_query($conn, "select * from fasilitas");
                                            while ($fetcharray = mysqli_fetch_array($ambildata)) {
                                                $nama_fasilitas = $fetcharray['nama_fasilitas'];
                                                $fasilitasid = $fetcharray['id'];
                                                $tempat_wisata_id = $fetcharray['tempat_wisata_id'];
                                            ?>
                                                <option value="<?= $fasilitasid; ?>"><?= $nama_fasilitas; ?></option>
                                            <?php
                                            }
                                            ?>
                                        </select>
